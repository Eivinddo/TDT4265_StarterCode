...
def evaluation(cfg, N_images: int):
    ...
    print("Number of trainable parameters in model:", sum(p.numel() for p in model.parameters() if p.requires_grad))
    ...
    file = open(cfg.output_dir.joinpath("logs/scalars.json"), 'r')
    mAP = []
    for line in file:
        data = json.loads(line)
        if 'metrics/mAP' in data:
            mAP.append(data['metrics/mAP'])
    mAP = torch.tensor(mAP)
    print("Max mAP:", float(torch.max(mAP)))
    print("Final mAP:", float(mAP[-1]))
